...
<h3 style="font-family: Georgia, 'Times New Roman', serif; font-size: 1.15em; font-weight: bold; color: #2c3e50; border-left: 4px solid #3498db; padding-left: 10px; margin-top: 1.5em;">两种部署方案 — 适配不同用户需求</h3>

MaskClaw 支持**企业级部署**与**个人部署**两种模式，用户可根据实际场景灵活选择：

| 维度 | 🏢 企业级部署 | 👤 个人部署 |
|:---|:---|:---|
| **部署位置** | 公司服务器，局域网/内网运行 | 个人设备本地运行 |
| **适用用户** | 公司员工，每人有独立的 Skills 库 | 普通个人用户 |
| **模型选择** | MiniCPM-V 4.5 | Gemma 4 2B |
| **硬件要求** | 服务器 GPU（推荐） | 无 GPU 要求，CPU 可运行 |
| **模型优势** | 语义理解强、适配迅敏、支持私有化定制 | 轻量快速、无需 GPU、资源占用低 |
| **数据归属** | 企业统一管理，支持审计追溯 | 数据完全私有，不出本地设备 |
| **协同能力** | 支持多用户协同过滤，规则共享加速收敛 | 单一用户规则自进化 |

> 💡 **企业用户**：推荐使用 MiniCPM-V 4.5，可快速适配公司业务需求，支持私有化部署与集中管理。
> 
...
| **Layer 2** | 认知层 (Cognition) | MiniCPM-V 4.5/Gemma 4 语义推理、ChromaDB RAG 规则检索 |
...
语义推理由 **MiniCPM-V 4.5/Gemma 4** 承担，视觉模型在端侧本地部署。敏感信息识别与视觉模糊处理全部在本地完成，不依赖网络连接。

| 组件 | 技术选型 | 优势 |
|:---|:---|:---|
| 视觉模型 | MiniCPM-V 4.5 (9B) / Gemma 4 2B | 端侧可部署，语义理解强 |
...
📥 **数据集下载**: [https://huggingface.co/datasets/Keonq/P-GUI-Evo](https://huggingface.co/datasets/Keonq/P-GUI-Evo) | CC BY 4.0 许可
