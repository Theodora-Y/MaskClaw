...
flowchart TB
    subgraph P["📱 感知层 (Perception)"]
        direction TB
        P1["🔍 RapidOCR<br/>毫秒级格式敏感识别"]
        P2["🎨 OpenCV<br/>本地视觉模糊处理"]
    end

    subgraph C["🧠 认知层 (Cognition)"]
        direction TB
        C1["🤖 MiniCPM-V 4.5<br/>语义推理 + Skill 调度"]
        C2["📚 ChromaDB<br/>RAG 规则检索"]
    end

    subgraph T["⚙️ 执行层 (Tool-Use)"]
        direction TB
        T1["🎭 Smart Masker<br/>视觉打码"]
        T2["🔒 PII Detection<br/>隐私检测"]
    end

    subgraph E["🔄 进化层 (Self-Evolution)"]
        direction TB
        E1["📊 Behavior Monitor<br/>行为监控捕获"]
        E2["🧬 Skill Evolution<br/>规则抽取生成"]
        E3["🧪 Sandbox<br/>沙盒回归验证"]
    end
